...
require 'rinruby'
def runR
 # if File.exist?('/home/deploy/apps/diffexdb/current/public/tn.html')
 # File.delete('/home/deploy/apps/diffexdb/current/public/tn.html')
 # end
  ...
  R.eval("j <- plot_ly(TU, y = TU$V2,type = 'box',name = 'Tumor', yaxis = list(type = 'log')) %>% add_trace(N,y = N$V2, name = 'Normal',yaxis = list(type = 'log')) %>% layout(title=sprintf('Expression Levels of %s \n P-Value  = %s ',names1,ttest$p.value))")
  ...
  R.eval('htmlwidgets::saveWidget(as_widget(j), "tn.html")')
  redirect_to('/tn.html')
...
def runRkaplansingle
 # if File.exist?('/home/deploy/apps/diffexdb/current/public/kaplanmeier.html')
#File.delete('/home/deploy/apps/diffexdb/current/public/kaplanmeier.html')
#end
